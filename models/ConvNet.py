...
class CIFAR10CNNConstLast(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.features = nn.Sequential(
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=False),
            nn.Conv2d(64, 64, kernel_size=3, padding=1, rank=64  ,),  # [Batch, 64, 32, 32]
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=False),
            nn.MaxPool2d(kernel_size=2, stride=2),        # [Batch, 64, 16, 16]

            # Conv Block 2
            nn.Conv2d(64, 128, kernel_size=3, padding=1, rank=128 ,), # [Batch, 128, 16, 16]
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=False),
            nn.Conv2d(128, 128, kernel_size=3, padding=1, rank=128 ,),# [Batch, 128, 16, 16]
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=False),
            nn.MaxPool2d(kernel_size=2, stride=2),        # [Batch, 128, 8, 8]

            # Conv Block 3
            nn.Conv2d(128, 256, kernel_size=3, padding=1, rank=256 ,),# [Batch, 256, 8, 8]
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=False),
            nn.Conv2d(256, 256, kernel_size=3, padding=1, rank=256 ,),# [Batch, 256, 8, 8]
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=False),
            nn.MaxPool2d(kernel_size=2, stride=2),        # [Batch, 256, 4, 4]

            # Conv Block 4
            nn.Conv2d(256, 512, kernel_size=3, padding=1, rank=rank,),# [Batch, 512, 4, 4]
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=False),
            nn.Conv2d(512, 512, kernel_size=3, padding=1, rank=rank,),# [Batch, 512, 4, 4]
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=False),
            # No MaxPool here to prevent reducing spatial dimensions too much
        )
