...
def extract_training_summary_to_csv(experiments, output_csv='training_summary.csv'):
    ...
    for task_name, exp_list in experiments.items():
        for exp_num, exp_folder in exp_list:
            try:
                ...
                # Find step where accuracy reached 95% of max
                target_acc = 0.1 * max_acc
                steps_90pct = []
                for step, acc in zip(acc_steps, acc_values):
                    if acc >= target_acc:
                        steps_90pct.append(step)
                step_90pct = min(steps_90pct)
                ...
                summary_data.append({
                    ...
                    'Step_90pct_Max': step_90pct if step_90pct else 'N/A'
                })
    ...
    if summary_data:
        fieldnames = ['Task', 'Exp_Number', 'Max_Val_Acc_Top1', 'Max_Val_Acc_Top1_Step', 
                      ...
                      'Last_Epoch_Step', 'Last_Epoch_Time', 'Step_90pct_Max']
...
def main():
    ...
    extract_training_summary_to_csv(experiments, os.path.join(save_dir, "training_summary_detailed.csv"))
    ...
    for task_name, exp_list in experiments.items():
        for exp_num, exp_folder in exp_list:
            try:
                ...
                # Find step where accuracy reached 90% of max
