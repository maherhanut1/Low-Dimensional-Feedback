...
import yaml
import os
from modules.bio_layers.LDFA_Linear import Linear as bio_LDFA_Linear
...
def main():
    ...
    device = 'cuda' # 'cuda' #'cuda' if torch.cuda.is_available() else 'cpu'
    # Data
    ...
    if use_ldfa_linear:
        replace_linear(model, bio_LDFA_Linear, rank=ldfa_rank)
    else:
    ...
    if use_ldfa_linear:
        ...
        modify_funcs = None #[lambda trainer: reinitialize_pq_layers(trainer, 0.5)]
        modification_rate = None #len(train_loader) // 2  # Reinit every half epoch
